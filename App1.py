...
""")
    ...
    if "genre" in st.session_state:
        ...
        st.write("❌ 장르 값 없음")
    # 🧭 사이드바에서 장르 재선택
    st.sidebar.header("🎯 유명인물의 추천도서")

    # 선택된 책 형태에 맞는 장르 목록 가져오기
    selected_book_type = st.session_state.get("book_types", "종이책")
    genre_options = GENRES_BY_BOOK_TYPE.get(st.session_state.get("book_types", "종이책"), [])
    st.sidebar.selectbox(
        "선호 장르를 선택해주세요",
        genre_options,
        key="genre"  # 👉 이 키만 사용하면 깔끔
    )

    # 추천 영역
    st.subheader("📚 베스트셀러 추천")
    selected_genre = st.session_state.genre  # 직접 가져와서 사용
    st.markdown(f"- 추천 책 리스트 ({selected_genre})")
